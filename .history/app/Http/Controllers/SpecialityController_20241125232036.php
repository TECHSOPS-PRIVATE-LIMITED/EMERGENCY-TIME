...
class SpecialityController extends Controller
{
    ...
    {
        // Validate that country_id is provided in the request
        $validated = $request->validate([
            'country_id' => 'required|exists:countries,id', // Ensure country_id exists in the countries table
        ]);

        $countryId = $validated['country_id']; // Retrieve the country_id after validation

        // Query the specialities based on the provided country_id
        ...
        $responseData = $specialities->map(function ($speciality) {
            return [
                ...
                'speciality_image' => Storage::url($speciality->speciality_image), // Get the URL for the image
            ];
        });
